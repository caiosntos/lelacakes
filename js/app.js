...
function showSuccessMessage() {
    ...
    notification.className = 'fixed top-24 right-8 bg-green-500 text-white px-6 py-4 rounded-2xl shadow-xl z-[110] transform translate-x-full transition-transform duration-300';
...
function removeItem(itemId) {
    const itemElement = document.getElementById(`qty-${itemId}`).closest('.bg-white');
...
function enableSection(stepNumber) {
    const sections = document.querySelectorAll('.bg-white.rounded-3xl.shadow-xl.p-8');
    ...
    if (options) {
        ...
        optionCards.forEach(card => {
            ...
            card.addEventListener('click', function() {
                const category = stepNumber === 2 ? 'recheio' : 
                               stepNumber === 3 ? 'topo' : 'decoracao';
                const value = this.querySelector('h3').textContent.toLowerCase().replace(' ', '');
